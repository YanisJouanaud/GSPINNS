...
class Optimizers(NamedTuple):
    optimizer_nn: optax.GradientTransformation
    optimizer_eq: optax.GradientTransformation
    every: tuple
...
def solve_alternate(
    ...
    if opt_state is None:
        opt_state_nn = optimizers[0].init(init_params)
        opt_state_eq = optimizers[1].init(init_params)
        opt_state = {"nn_params":opt_state_nn, "eq_params":opt_state_eq}
    ...
    def one_iteration(carry):
        ...
        (
            ...
            opt_state,
            last_non_nan_params,
        ) = jax.lax.cond(i % (every[0] + every[1]) < every[0],
        lambda _:gradient_step_nn(
            loss,
            optimizers[0],
            batch,
            optimization.params,
        ...
        return (
            ...
            OptimizationExtraContainer(None, None, early_stopping),
...
@partial(jit, static_argnames=["optimizer", "param_choice"])
def gradient_step_choice(loss, optimizer, batch, params, opt_state, last_non_nan_params, param_choice):
    """
    optimizer cannot be jit-ted.
    """
    # jax.debug.print(param_choice)
    opt_state_fn = opt_state[param_choice]
    loss.derivative_keys = {"dyn_loss":[param_choice], 
                            "initial_conditions":[param_choice], 
                            "observations":[param_choice]}
    # only compute gradient w.r.t to params["nn_params"]
    value_grad_loss = jax.value_and_grad(loss, has_aux=True)
    (loss_val, loss_terms), grads = value_grad_loss(params, batch)

    updates, opt_state_fn = optimizer.update(grads, opt_state_fn, params)

    params = optax.apply_updates(params, updates)
    opt_state[param_choice] = opt_state_fn
    ...
    loss.derivative_keys = {"dyn_loss":["eq_params", "nn_params"], 
...
def gradient_step_nn(loss, optimizer, batch, params, opt_state, last_non_nan_params):
    """
    optimizer cannot be jit-ted.
    """

    param_choice = "nn_params"
    opt_state_fn = opt_state[param_choice]
    loss.derivative_keys = {"dyn_loss":[param_choice], 
                            "initial_conditions":[param_choice], 
                            "observations":[param_choice]}
...
@partial(jit, static_argnames=["optimizer"])
def gradient_step_eq(loss, optimizer, batch, params, opt_state, last_non_nan_params):
    """
    optimizer cannot be jit-ted.
    """

    param_choice = "eq_params"
    opt_state_fn = opt_state[param_choice]
    loss.derivative_keys = {"dyn_loss":[param_choice], 
                            "initial_conditions":[param_choice], 
                            "observations":[param_choice]}
